...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///test.db" # connect to the db
app.config["GOOGLEMAPS_KEY"] = "YOUR_GOOGLE_MAPS_API_KEY"
app.secret_key = b'kjkl3jk35jk3xciifo'
db = SQLAlchemy(app)
GoogleMaps(app) # for the python google maps api wrapper
...
def index():
    ...
    cur_loc = get_cur_loc()
    # create the Map for the googlemaps python api
    sndmap = Map(
        identifier="sndmap",
        lat=cur_loc[0],
        lng=cur_loc[1],
...
def login():
    if request.method == 'POST':
        ...
        print(results)
        # if there is at least one result in the database, the user and pass has been validated so create a session
        if len(results) >= 1:

            # create the session variables for username
    ...
    # when method = "GET"
        ...
        # when the already login redirect them back index homepage
        if ("logged_in" in session) and session["logged_in"]:
            return redirect('/index')
...
def get_db():
    '''
    Return List[tuple] where each tuple is a row of the sqlite database
    '''
    sql = """
        SELECT *
        FROM company_info
...
def get_cur_loc():
    '''
    Return List[float] where 0 index is the lat and 1 index is the long
    '''
    myloc = geocoder.ip('me') # pulls cur location based off IP. Might be inaccurate in certain cases
    print(myloc.latlng)
